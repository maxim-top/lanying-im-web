<template>
  <div class="add_friend_layer">
    <div class="layer">
      <div class="layer_header">加入群
        <div
          @click="clickJoinGroupCloseHandler"
          class="closer"
        >
        </div>
      </div>
...
export default {
  ...
  computed: {
    ...
    isJoined() {
      return (
        this.getGroupList.findIndex(
          x => x.group_id === this.searchObj.group_id
        ) >= 0
      );
    }
  },
  methods: {
    ...
    clickJoinHandler() {
      const group_id = this.group_id - 0;
      this.$store.getters.im.groupManage
        .asyncApply({ group_id, reason: "申请加入群" })
        .then(() => {
          alert("请求已发送成功!");
        });
    }
